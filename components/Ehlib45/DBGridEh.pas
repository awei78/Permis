...
//  THorzCellAreaTypeEh = (hctIndicatorEh, hctFrozenEh, hctDataEh, hctContraEh);
  ...
  TDBGridEhOption = (dghFixed3D, dghFrozen3D, dghFooter3D, dghData3D, dghResizeWholeRightPart,
    ...
    dghRecordMoving, dghShowRecNo, dghColumnResize, dghColumnMove, dghAutoFitRowHeight);
...
//gridseh    procedure SetFlat(const Value: Boolean);
    ...
    procedure UpdateIme;
    procedure UpdateColumnResizeOptions(const NewOptions: TDBGridOptions; const NewOptionsEh: TDBGridEhOptions);
{$IFDEF EH_LIB_VCL}
...
{$ENDIF}
    ...
    procedure FillCellRect(DrawByThemes: Boolean; ARect: TRect; IsDown, IsSelected: Boolean; ClipRect: TRect; Cell3D: Boolean);
...
begin
  ...
  begin
    ...
    if (dghRowHighlight in AGrid.OptionsEh) and (DataRow + AGrid.TopDataOffset = AGrid.Row) and
      (AGrid.Selection.SelectionType = gstNon) and not (DataCol + AGrid.FIndicatorOffset = AGrid.Col)
...
{          if not Assigned(CurColumn.OnNotInlist) or
      ...
      begin
        with Grid, CurColumn.Field do
          ...
          begin
            DataSet.Edit;
            VarDateTime := Trunc(FPopupMonthCalendar.Date);
...
begin
  ...
  WordWrap := Column.WordWrap and Column.CurLineWordWrap(Grid.RowHeights[Grid.Row]);
...
//  FTopDataOffset := GetTopDataOffset;
  ...
  FOptionsEh := [dghFixed3D, dghHighlightFocus, dghClearSelection,
    dghDialogFind, dghColumnResize, dghColumnMove];
...
//    InflateRect(ARect, 1, 1);
  ...
  if (CellAreaType.VertType = vctTitleEh) and (FUseMultiTitle = True) then
    with DrawColumn.Title do
    begin // Draw mastertitle
      Canvas.Font := TitleFont;
      if Highlight then
...
begin
  ...
  if (FIntMemTable <> nil) and (AreaCell.X = 0) and FIntMemTable.MemTableIsTreeList then
  begin
...
//  ACol, ARow, AreaCol, AreaRow: Integer;
  ...
  begin
    ...
    DrawFlag := AlignFlags[Alignment];
    if Column.WordWrap and Column.CurLineWordWrap(RowHeights[Cell.Y]) then
      DrawFlag := DrawFlag or DT_WORDBREAK or RTL[UseRightToLeftReading];
...
//      InflateRect(ATextRect, -1, -1);
  ...
  AButtonsWidth := 0;
  ACellRect := AreaRect;
  if Column.AlwaysShowEditButton and not FColCellParamsEh.BlankCell then // Draw edit button
  begin
  ...
  begin
    IsFillTextCell := The3DRect and not (gdSelected in FColCellParamsEh.State) and
    ...
    begin
      ...
      with FColCellParamsEh do
        WriteCellText {WriteTextEh}(Column, Canvas, ARect1, not IsFillTextCell,
          XFrameOffs, YFrameOffs, FText,
          FAlignment, Column.Layout, Column.WordWrap and Column.CurLineWordWrap(RowHeights[Cell.Y]),
  ...
  if not FColCellParamsEh.BlankCell and Column.AlwaysShowEditButton then // Draw edit button
  begin
...
//    and (gdSelected in AState) KMV #12
  ...
  if not FColCellParamsEh.BlankCell and
         Column.AlwaysShowEditButton and
...
procedure TCustomDBGridEh.FillCellRect(DrawByThemes: Boolean; ARect: TRect;
  IsDown, IsSelected: Boolean; ClipRect: TRect; Cell3D: Boolean);
{$IFDEF EH_LIB_7}
var
  Details: TThemedElementDetails;
begin
...
begin
  ...
  begin
    ...
    if (FUseMultiTitle = True) and DoClick and (Cell.X - IndicatorOffset >= 0) then
    begin
      ARect := CellRect(Cell.X, Cell.Y);
...
procedure TCustomDBGridEh.UpdateColumnResizeOptions(const NewOptions: TDBGridOptions;
  const NewOptionsEh: TDBGridEhOptions);
begin
  ...
  if (FOptions = NewOptions) and (dgColumnResize in FOptions) then
    FOptionsEh := FOptionsEh + [dghColumnResize, dghColumnMove];
      ...
      giNormalArrowEh, giSelectedArrowEh, giInplaceSearch:
        begin
          ...
          FillGradientEh(Canvas,
            Point((ARect.Right + ARect.Left - DBGridEhIndicators.Width) shr 1 + XOffset,
            (ARect.Top + ARect.Bottom - DBGridEhIndicators.Height) shr 1 + YOffset),
            [Point(0,0), Point(1,0),
             Point(0,1), Point(2,1),
             Point(0,2), Point(3,2),
             Point(0,3), Point(4,3),
...
begin
  if (Message.Msg = WM_LBUTTONDOWN) and (csDesigning in ComponentState) and
...
procedure TCustomDBGridEh.DefaultIndicatorTitleMouseDown(Cell: TGridCoord; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  DropdownMenu: TPopupMenu;
  P: TPoint;
