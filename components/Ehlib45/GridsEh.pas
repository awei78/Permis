...
{$ELSE}
  EhLibVCL,
{$ENDIF}
  Graphics, Menus, Controls, Forms, StdCtrls, Mask, Dialogs;
...
type
  ...
  protected
    ...
    FDataPageWidth: Integer;
    FFixedLineColor: TColor;
    FGridDataWidth: Integer;
    FGridState: TGridState;
    FHorzSmoothPos: Integer;
    FLockPaint: Integer;
    ...
    procedure SmoothScrollHorzData(DX: Integer);
...
// Data cells
      DrawLines(DrawInfo, goHorzLine in Options, goVertLine in Options, LeftCol, TopRow,
        [Horz.OutViewVisCellStart, Vert.FixedBoundary, Horz.GridBoundary, Vert.GridBoundary],
...
//Top cells in data cols
    ...
    DrawCells(LeftCol, 0, Horz.OutViewVisCellStart, 0, Horz.GridBoundary,  //!! clip
      Vert.NoFrozenBoundary, FixedColor, [gdFixed]);
    DrawCells(LeftCol, FixedRows-FrozenRowCount,
      Horz.OutViewVisCellStart, Vert.NoFrozenBoundary, Horz.GridBoundary,  //!! clip
...
var
  ...
  procedure ModifySmoothScrollBar(Code, Pos: Cardinal);
  var
    NewOffset: Integer;
  begin
    NewOffset := 0;
    case Code of
      ...
      SB_PAGEUP: NewOffset := -FDataPageWidth * RTLFactor;
      SB_PAGEDOWN: NewOffset := FDataPageWidth * RTLFactor;
      SB_THUMBPOSITION,
      SB_THUMBTRACK:
        if (goThumbTracking in Options) or (Code = SB_THUMBPOSITION) then
        begin
          if not UseRightToLeftAlignment then
            NewOffset := Integer(Pos) - FHorzSmoothPos
          else
            NewOffset := FHorzSmoothPos - Integer(Pos);
        end;
      SB_BOTTOM: NewOffset := 0;
      SB_TOP: NewOffset := FGridDataWidth - FDataPageWidth;
    end;
    SmoothScrollHorzData(NewOffset);
  end;
...
begin
  ...
  if (ScrollBar = SB_HORZ) then
    ...
    begin
      ModifySmoothScrollBar(ScrollCode, Pos);
      Exit;
...
begin
  ...
  begin
    Inc(FGridDataWidth, ColWidths[i] + GridLineWidth);
    if i < LeftCol then
      FHorzSmoothPos := FGridDataWidth;
  end;
  CalcDrawInfo(DrawInfo);
  FDataPageWidth := DrawInfo.Horz.GridExtent - DrawInfo.Horz.FixedBoundary;
  if FDataPageWidth < 0 then FDataPageWidth := 0;
end;
...
begin
  ...
  then
    if (FColDataOffset <> 0) and (OldTopLeft.X <> FTopLeft.X) then
    begin
      Delta.X := Delta.X + FColDataOffset;
      FColDataOffset := 0;
    end;
  ...
  if ScrollBars in [ssHorizontal, ssBoth] then
    ...
    begin
      ...
      for i := FixedCols to LeftCol-1 do
        Inc(FHorzSmoothPos, ColWidths[i] + GridLineWidth);
      Inc(FHorzSmoothPos, FColDataOffset);
      if UseRightToLeftAlignment then
        FHorzSmoothPos := FGridDataWidth - FHorzSmoothPos - FDataPageWidth;
      SetGridScrollInfo(SB_HORZ, 0, FGridDataWidth-1, FDataPageWidth, FHorzSmoothPos);
    end else if ColCount = 1 then
    begin
      ColWidth := ColWidths[DrawInfo.Horz.FirstGridCell];
      GridSpace := ClientWidth - DrawInfo.Horz.FixedBoundary;
...
//      SetScrollRange(Handle, Code, 0, 0, True);
  ...
  begin
    ...
    if FHorzSmoothPos > MaxSmoothPos then
      SmoothScrollHorzData(MaxSmoothPos - FHorzSmoothPos);
    FDataPageWidth := DrawInfo.Horz.GridExtent - DrawInfo.Horz.FixedBoundary;
    if FDataPageWidth < 0 then FDataPageWidth := 0;
    SetGridScrollInfo(SB_HORZ, 0, FGridDataWidth-1, FDataPageWidth, FHorzSmoothPos);
  end;
...
//          Dec(NewCurrent.Y, RTLFactor) else
          SmoothScrollHorzData(-GetHorzScrollStep);
          Exit;
        end else
    ...
    begin
      ...
      begin
        ...
        Exit;
         ...
         ( (OldLeftCol < LeftCol) or (OldHorzSmoothPos < FHorzSmoothPos) ) then
      begin
  ...
  begin
    ...
    case FGridState of
      gsColMoving:
        begin
          OldLeftCol := LeftCol;
          OldHorzSmoothPos := FHorzSmoothPos;
